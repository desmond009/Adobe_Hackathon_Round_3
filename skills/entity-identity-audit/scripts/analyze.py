#!/usr/bin/env python3
"""CLI entrypoint for the entity-identity-audit skill.
...
from audit_engine.analyzers import entity_identity as analyzer
...
def main() -> None:
    args = build_cli_parser("entity-identity-audit").parse_args()
    ...
    for i, candidate in enumerate(analyzer.analyze(snapshot, cfg), start=1):
        finding = build_finding(candidate, f"EI-{i:03d}", threshold)
        if finding:
