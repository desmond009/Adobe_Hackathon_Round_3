#!/usr/bin/env python3
"""CLI entrypoint for the structured-data-audit skill.
...
from audit_engine.analyzers import structured_data_audit as analyzer
...
def main() -> None:
    args = build_cli_parser("structured-data-audit").parse_args()
    ...
    for i, candidate in enumerate(analyzer.analyze(snapshot, cfg), start=1):
        finding = build_finding(candidate, f"SD-{i:03d}", threshold)
        if finding:
