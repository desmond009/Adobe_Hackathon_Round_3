#!/usr/bin/env python3
"""CLI entrypoint for the engagement-audit skill.
...
from audit_engine.analyzers import engagement as analyzer
...
def main() -> None:
    args = build_cli_parser("engagement-audit").parse_args()
    ...
    for i, candidate in enumerate(analyzer.analyze(snapshot, cfg), start=1):
        finding = build_finding(candidate, f"EN-{i:03d}", threshold)
        if finding:
