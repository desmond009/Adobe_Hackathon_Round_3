#!/usr/bin/env python3
"""CLI entrypoint for the content-extractability-audit skill.
...
from audit_engine.analyzers import content_extractability as analyzer
...
def main() -> None:
    args = build_cli_parser("content-extractability-audit").parse_args()
    ...
    for i, candidate in enumerate(analyzer.analyze(snapshot, cfg), start=1):
        finding = build_finding(candidate, f"CE-{i:03d}", threshold)
        if finding:
